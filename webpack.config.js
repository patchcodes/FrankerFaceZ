...
console.log(
	"FOR_EXTENSION:",
	FOR_EXTENSION,
	FOR_EXTENSION ? ` (${process.env.FFZ_EXTENSION})` : ""
);
...
if (FOR_EXTENSION) ENTRY_POINTS.worker = "./src/worker.ts";

const COPY_PATTERNS = [
	{
		from: FOR_EXTENSION ? "./src/entry_ext.js" : "./src/entry.js",
		to: DEV_SERVER || DEV_BUILD ? "script.js" : "script.min.js",
	},
];
...
const config = {
	mode: DEV_BUILD ? "development" : "production",
	devtool: DEV_BUILD ? "inline-source-map" : "source-map",
	...
	output: {
		...
		clean: true,
		publicPath: FOR_EXTENSION ? "auto" : FILE_PATH,
		path: path.resolve(__dirname, "dist"),
		filename: "[name].js",
		// (FOR_EXTENSION || DEV_SERVER)
		// 	? '[name].js'
	...
	plugins: [
		...
		new EsbuildPlugin({
			define: {
				...
				__version_build__: JSON.stringify(
					process.env.FFZ_BUILD || null
				),
