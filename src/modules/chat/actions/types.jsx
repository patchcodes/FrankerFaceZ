...
export const pin = {
	...
	tooltip(data) {
		const pinned =
			data.line?.props?.pinnedMessage?.message?.id === data.message_id;
		if (pinned)
			return this.i18n.t(
				"chat.actions.pin.already",
	...
	disabled(data, message, current_room, current_user, mod_icons, instance) {
		const line = instance ?? data.line,
			props = line?.props,
			pinned =
				props?.pinnedMessage?.message?.id === message.id &&
				message.id != null;
	...
	hidden(data, message, current_room, current_user, mod_icons, instance) {
		...
		if (!line.props.isPinnable || !line.onPinMessageClick) return true;
	...
	click(event, data) {
		...
		if (!line) {
			...
			line = fine
				? fine.searchParent(
						event.target,
						(n) => n.setMessageTray && n.props && n.props.message
				  )
				: null;
		}

		if (!line || !line.props.isPinnable || !line.onPinMessageClick) return;

		if (line.props.pinnedMessage?.message?.id === data.message_id) return;
...
export const reply = {
	...
	hidden(data, message, current_room, current_user) {
		...
		if (!current_user?.can_reply) return true;

		// If reply mode is set to 0 (Disabled), don't show the action.
		if (current_user?.reply_mode === 0) return true;

		// If the message is empty or deleted, don't show the action.
		if (!message.message || message.deleted) return true;
	},
...
export const edit_overrides = {
	...
	description:
		"Allows you to set local overrides for a user's name and color in chat.",

	can_self: true,

	tooltip() {
		return this.i18n.t(
			"chat.actions.edit_overrides",
			"Change Name & Color"
		);
	},
	...
	click(event, data) {
		...
		this.resolve("chat.overrides").renderUserEditor(
			data.user,
			event.target
		);
	},
...
export const copy_message = {
	...
	editor: () =>
		import(
			/* webpackChunkName: 'main-menu' */ "./components/edit-copy.vue"
		),
...
export const open_url = {
	...
	editor: () =>
		import(/* webpackChunkName: 'main-menu' */ "./components/edit-url.vue"),
...
export const post_url = {
	...
	editor: () =>
		import(
			/* webpackChunkName: 'main-menu' */ "./components/streamerbot-action.vue"
		),
	...
	tooltip(data) {
		...
		return [
			<div class="tw-border-b tw-mg-b-05">Run StreamerBot Action</div>,
			<div class="tw-align-left">Action ID: {actionId}</div>,
		];
	},
...
export const chat = {
	...
	description(data) {
		...
		return this.t(
			"chat.actions.chat.desc.target",
			"Send in {target}: {cmd}",
			{
				cmd: data.options.command,
				target: /^\s*$/.test(target)
					? this.t(
							"chat.actions.chat.desc.current",
							"current channel"
					  )
					: target,
			}
			...
			<div class="tw-border-b tw-mg-b-05">
				{
					...
					target
						? this.i18n.t(
								"chat.actions.chat.with-target",
								"Chat Command in Channel: {target}",
								{ target }
						  )
	...
	click(event, data) {
		...
		if (data.options.paste) this.pasteMessage(target, msg);
		else this.sendMessage(target, msg);
	},
};
...
export const msg_delete = {
	...
	tooltip(data) {
		return this.i18n.t(
			"chat.actions.delete.tooltip",
			"Delete {user.login}'s message",
			{ user: data.user }
		);
	},
	...
	click(event, data) {
		...
		return td
			.deleteChatMessage(data.room.id, data.message_id)
			.catch((err) => {
...
export const ban = {
	...
	editor: () =>
		import(/* webpackChunkName: 'main-menu' */ "./components/edit-ban.vue"),

	title: "Ban User",

	reason_text(data) {
		return this.i18n.t("chat.actions.ban-reason", "Ban {user.login} for:", {
			user: data.user,
		});
	},

	tooltip(data) {
		return this.i18n.t("chat.actions.ban.tooltip", "Ban {user.login}", {
			user: data.user,
		});
	},
	...
	click(event, data) {
		...
		if (reason.length) reason = this.replaceVariables(reason, data);
...
export const timeout = {
	...
	editor: () =>
		import(
			/* webpackChunkName: 'main-menu' */ "./components/edit-timeout.vue"
		),
	...
	click(event, data) {
		...
		this.sendMessage(
			data.room.login,
			`/timeout ${data.user.login} ${data.options.duration} ${reason}`
		);
	},
};
...
export const unban = {
	...
	tooltip(data) {
		return this.i18n.t("chat.actions.unban.tooltip", "Unban {user.login}", {
			user: data.user,
		});
	},
...
export const untimeout = {
	...
	tooltip(data) {
		return this.i18n.t(
			"chat.actions.untimeout.tooltip",
			"Untimeout {user.login}",
			{ user: data.user }
		);
	},
...
export const mod = {
	...
	tooltip(data) {
		return this.i18n.t("chat.actions.mod.tooltip", "Mod {user.login}", {
			user: data.user,
		});
	},
	...
	hidden(data, message, current_room, current_user, mod_icons, instance) {
		// You cannot mod mods.
		if (message.user.type === "mod") return true;

		// You cannot mod the broadcaster.
		if (message.user.id === current_room.id) return true;
...
export const unmod = {
	...
	hidden(data, message, current_room, current_user, mod_icons, instance) {
		...
		if (message.user.id === current_user.id) return false;

		// You cannot unmod the broadcaster.
		if (message.user.id === current_room.id) return false;
...
export const toggle_profile = {
	...
	editor: () =>
		import(
			/* webpackChunkName: 'main-menu' */ "./components/edit-profile.vue"
		),
	...
	description(data) {
		...
		return profile.i18n_key
			? this.t(profile.i18n_key, profile.name)
			: profile.name;
	},
	description_i18n: null,
	...
	tooltip(data) {
		...
		if (!profile) name = "invalid";
			...
			name = profile.i18n_key
				? this.i18n.t(profile.i18n_key, profile.name)
				: profile.name;

		return this.i18n.t(
			"chat.actions.toggle_profile.tooltip",
			"Toggle Profile: {name}",
			{
				name,
			}
				...
				for (const item of stuff) {
					if (item.action === "toggle_profile") {
						if (item.options?.uuid === profile.uuid) {
							found.add(ctx);
							this.parent.context.once(`changed:${key}`, () =>
								found.delete(ctx)
							);
							break;
						}
